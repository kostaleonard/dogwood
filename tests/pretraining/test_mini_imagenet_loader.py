...
def test_download_mini_imagenet_retrieves_files() -> None:
    ...
    assert os.path.exists(os.path.join(
        TEST_IMAGENET_PATH, 'imagenet-mini', 'train'))
    assert len(os.listdir(os.path.join(
        TEST_IMAGENET_PATH, 'imagenet-mini', 'train'))) == IMAGENET_CLASSES
    assert os.path.exists(os.path.join(
        TEST_IMAGENET_PATH, 'imagenet-mini', 'val'))
    assert len(os.listdir(os.path.join(
        TEST_IMAGENET_PATH, 'imagenet-mini', 'val'))) == IMAGENET_CLASSES
    shutil.rmtree(TEST_IMAGENET_PATH, ignore_errors=True)
