...
def fixture_mnist_dataset() -> tuple[
...
def fixture_micro_symmetry_dataset() -> tuple[np.ndarray, np.ndarray]:
    ...
    num_examples = 5
    X_train = np.arange(
        num_examples * MICRO_INPUT_LEN, dtype=np.float32
    ).reshape((num_examples, MICRO_INPUT_LEN))
...
def fixture_mnist_versioned_model(
    mnist_versioned_dataset: VersionedDataset, mnist_model: Sequential
) -> VersionedModel:
